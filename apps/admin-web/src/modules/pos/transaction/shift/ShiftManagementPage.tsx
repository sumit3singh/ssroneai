...
export const ShiftManagementPage: React.FC = () => {
  ...
  const mockShift: POSShiftSummary = {
    shift_id: "SH-2026-0728-01",
  ...
  const expectedCash = mockShift.opening_balance + mockShift.cash_sales;
    ...
    <div className="bg-card border border-border rounded-2xl p-6 shadow-card space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-muted/30 border border-border/80 rounded-xl p-4 space-y-1">
          ...
          <p className="text-lg font-black text-foreground font-mono">₹{mockShift.opening_balance}</p>
        </div>

        <div className="bg-muted/30 border border-border/80 rounded-xl p-4 space-y-1">
          <span className="text-2xs font-extrabold text-muted-foreground uppercase">Live Cash Collections</span>
          <p className="text-lg font-black text-emerald-600 dark:text-emerald-400 font-mono">₹{mockShift.cash_sales}</p>
        </div>

        <div className="bg-muted/30 border border-border/80 rounded-xl p-4 space-y-1">
          <span className="text-2xs font-extrabold text-muted-foreground uppercase">Digital Sales (UPI/Card)</span>
          <p className="text-lg font-black text-blue-600 dark:text-blue-400 font-mono">₹{mockShift.upi_sales + mockShift.card_sales}</p>
        </div>
