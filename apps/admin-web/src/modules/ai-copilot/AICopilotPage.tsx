...
import { getAccessToken } from "@/shared/utils/api-client";
...
function generateMockResponse(query: string, _agent: string): string {
  if (!isMockSession()) {
    return "This AI Copilot simulation is only available in a development mock session.";
  }

  const q = query.toLowerCase();

  if (q.includes("revenue") || q.includes("sales") || q.includes("profit") || q.includes("finance")) {
    const orders = mockDB.get<any>("orders");
    const sales = orders.reduce((sum: number, o: any) => sum + (o.payment_status === "paid" ? o.grand_total : 0), 0);
    const active = orders.length;
    return `According to current live financial records, total sales revenue collected is **${formatCurrency(sales)}** across **${active} orders**. Net profit margins are estimated at **34.2%** after adjusting for payroll costs and ingredient inventories.`;
  }

  if (q.includes("stock") || q.includes("inventory") || q.includes("ingredient")) {
    const stocks = mockDB.get<any>("stocks");
    const low = stocks.filter((s: any) => s.quantity <= s.reorder_level);
    if (low.length > 0) {
      return `Our inventory records show **${low.length} items below minimum reorder thresholds**:\n\n${low.map((s: any) => `- **${s.name}**: Current Stock is **${s.quantity} ${s.unit}** (reorder level: ${s.reorder_level})`).join("\n")}\n\nI recommend initiating a purchase order today.`;
    }
    return `All ingredient stocks are currently within healthy operational levels. No immediate purchasing shortages detected.`;
  }

  if (q.includes("room") || q.includes("occupancy") || q.includes("hotel")) {
    const rooms = mockDB.get<any>("rooms");
    const total = rooms.length;
    const occupied = rooms.filter((r: any) => r.status === "occupied").length;
    const rate = total > 0 ? Math.round((occupied / total) * 100) : 0;
    return `Hotel PMS reports **${occupied} occupied rooms** out of **${total} total rooms**, representing a **${rate}% room occupancy rate** today. We have **${rooms.filter((r: any) => r.status === "cleaning").length} rooms currently marked for cleaning**.`;
  }

  if (q.includes("resident") || q.includes("pg") || q.includes("hostel")) {
    const residents = mockDB.get<any>("residents");
    const overdue = residents.filter((r: any) => r.paid_status === "overdue");
    return `PG hostel has **${residents.length} active residents**. There are **${overdue.length} residents with rent overdue**:\n\n${overdue.map((r: any) => `- **${r.name}** (Room: ${r.room}, Bed: ${r.bed}): **${formatCurrency(r.rent)}** overdue`).join("\n")}\n\nYou can initiate collection notifications from the PG Management panel.`;
  }

  return `The Baithak Copilot is operating in local simulation mode. I can see you are asking about "${query}". In a fully connected setup, I query live Spanner databases and run advanced RAG search protocols. You can ask me about "revenue", "stock levels", "hotel occupancy", or "PG resident rent statuses" to see live local analysis!`;
}
...
export function AICopilotPage() {
  ...
  const [messages, setMessages] = useState<ChatMsg[]>([
    {
      role: "assistant",
      content:
        "Hi! I'm The Baithak AI Copilot. I can help you analyze revenue, forecast inventory, understand customer trends, and answer questions about your business. What would you like to know?",
    },
  ]);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (isMock) {
      ...
      const fullResponse = generateMockResponse(userMessage, activeAgent);
      let index = 0;
      ...
      const interval = setInterval(() => {
        ...
        index++;
        if (index >= fullResponse.split(" ").length) {
