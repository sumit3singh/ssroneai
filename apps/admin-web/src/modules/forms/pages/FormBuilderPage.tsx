...
export function FormBuilderPage() {
  ...
  useEffect(() => {
    api.get<any>(`/forms/${formKey}`).then(setFormDef).catch(() => setFormDef(null));
    api.get<any[]>(`/forms/${formKey}/submissions`).then((res) => Array.isArray(res) && setSubmissions(res)).catch(() => setSubmissions([]));
  }, [formKey]);
  ...
  const handleAddField = async (e: React.FormEvent) => {
    ...
    try {
      await api.post(`/forms/${formKey}/fields`, {
        ...newField,
        name: formattedFieldName
      });
      toast.success(`New field "${newField.label}" added to ${formKey}`);
    } catch {
  ...
  const handleDeleteField = async (fieldId: string, label: string) => {
    if (confirm(`Are you sure you want to delete the metadata field "${label}"?`)) {
      try {
        await api.delete(`/forms/${formKey}/fields/${fieldId}`);
  ...
  const handleDeleteSubmission = async (subId: string) => {
    if (confirm("Permanently delete this form submission record?")) {
      try {
        await api.delete(`/forms/submissions/${subId}`);
  ...
  useEffect(() => {
    api.get<any[]>("/forms").then((res) => Array.isArray(res) && setAllForms(res)).catch(() => setAllForms([]));
  }, []);
