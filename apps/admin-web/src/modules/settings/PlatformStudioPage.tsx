...
const INITIAL_ORG_HIERARCHY = {
  id: "org-baithak",
  name: "The Baithak Group (Org)",
  type: "organization",
  manager: "Sumit Singh (CEO)",
  children: [
    {
      id: "brand-restro",
      name: "Baithak Sweets & Restro",
      type: "brand",
...
export function PlatformStudioPage() {
  ...
  const [lovs, setLovs] = useState<any[]>(() => isMock ? mockDB.get<any>("lovs") : []);
  ...
  const [orgTree, setOrgTree] = useState<any>(() => {
    const saved = localStorage.getItem("baithak_org_hierarchy");
  ...
  const saveOrgTree = (updatedTree: any) => {
    ...
    localStorage.setItem("baithak_org_hierarchy", JSON.stringify(updatedTree));
  };
          ...
          <table className="w-full text-left border-collapse text-xs">
            ...
            <tbody className="divide-y divide-border/60 font-semibold text-muted-foreground">
              {[
                ...
                { date: "2026-07-13 14:00", module: "Core DB", action: "Write mockDB local fallback cache", status: "Success" },
              ].map((l, idx) => (
  ...
  const handleAddLov = (e: React.FormEvent) => {
    e.preventDefault();
    if (!isMock) {
      toast.error("LOV management is only available in mock sessions.");
      return;
    }
    ...
    if (editingLov) {
      // Edit mode
      mockDB.update("lovs", editingLov.id, {
        label: lovForm.label,
            ...
            <form onSubmit={handleAddChildNode}>
              <div className="p-5 space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    required
                    placeholder={`e.g. ${newNodeForm.type === "brand" ? "Baithak Banquets" :
