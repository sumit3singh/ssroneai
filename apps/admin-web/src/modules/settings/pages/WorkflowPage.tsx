...
export function WorkflowPage() {
  ...
  const handleProcessRequest = async (statusAction: "approved" | "rejected") => {
    if (!selectedReq) return;

    try {
      await api.post(`/workflow/requests/${selectedReq.id}/action`, {
        action: statusAction,
        remarks: remarks || undefined
      });
      toast.success(`Request ${statusAction} successfully!`);
      setRemarks("");
      refreshRequests();
    } catch {
      toast.error(`Action executed for request.`);
    }
  };
  ...
  const handleAddTemplate = async (e: React.FormEvent) => {
    ...
    try {
      const created = await api.post<any>("/workflow/templates", {
  ...
  const handleAddStep = async (e: React.FormEvent) => {
    ...
    try {
      await api.put(`/workflow/templates/${selectedTpl.id}`, updated);
