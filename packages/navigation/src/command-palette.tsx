...
export function CommandPalette() {
    ...
    }, [open, filteredPages, selected, execute]);
    ...
    const grouped = filteredPages.reduce<Record<string, CommandItem[]>>((acc, cmd) => {
                    ...
                    {filteredPages.length === 0 ? (
                        ...
                        Object.entries(grouped).map(([category, items]) => (
                            <div key={category} className="mb-2">
                                ...
                                {items.map((cmd) => {
                                    const idx = filteredPages.indexOf(cmd);
                                    return (
                                        <button
                                            key={cmd.id}
                                            onClick={() => execute(cmd)}
                                        ...
                                        >
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                {cmd.description && (
                                                    <p className={cn("text-xs truncate", selected === idx ? "text-white/70" : "text-muted-foreground")}>{cmd.description}</p>
                                                )}
                                            </div>
