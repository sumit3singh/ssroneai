/**
 * SSR One AI – Command Palette (Ctrl+K)
 * Global command palette per SSR One AI Enterprise UX Standards.
...
const BASE_URL = (typeof import.meta !== "undefined" && (import.meta as any).env?.VITE_API_URL) ?? "http://localhost:8000/api/v1";
...
export function CommandPalette({ onNavigate }: { onNavigate?: (href: string) => void }) {
  ...
  const execute = useCallback((cmd: CommandItem) => {
    ...
    if (cmd.href) {
      ...
      else if (typeof window !== "undefined") window.location.href = cmd.href;
    } else if (cmd.action) {
      cmd.action();
    }
  }, [onNavigate]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[20vh]">
      ...
      />
      <div className="relative w-full max-w-xl mx-4 rounded-2xl border border-border bg-card shadow-lg overflow-hidden">
        <div className="flex items-center gap-3 px-4 py-3.5 border-b border-border">
          ...
          />
          <kbd className="flex-shrink-0 text-xs bg-muted border border-border px-1.5 py-0.5 rounded text-muted-foreground">
            ESC
          </kbd>
        </div>
